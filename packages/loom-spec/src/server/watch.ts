...
export interface LoomChangeEvent {
  type: "diagram-changed" | "node-types-changed" | "journey-changed";
  id?: string;
}
    ...
    try {
      this.watcher = chokidarWatch(
        [
          ...
          resolve(this.loomPath, "journeys"),
      ...
      this.watcher.on("all", (_eventName: string, path: string) => {
        ...
        } else if (path.endsWith(".journey.json")) {
          const id = basename(path, ".journey.json");
          this.emit("change", { type: "journey-changed", id });
