...
**v0.5.0 published on npm** ([npmjs.com/package/loom-spec](https://www.npmjs.com/package/loom-spec)). Real-world use confirmed (the author's day-to-day project).

v0.5.0 is a **deliberate scope-down**. The timeline view (built across v0.2.0–v0.3.0 as a DAW-style architecture playback) was removed in v0.5.0 because the only confirmed user didn't use it. Same for `loom-spec import-trace` (OTel JSON → timeline) and the 5 timeline-related MCP tools. The conceptual surface drops by ~30%, the export bundle shrinks, and the product story sharpens to "agent-readable architecture spec exportable as interactive HTML, with guided walkthroughs of common workflows".
...
Next planned chunk: **[Journeys](./journeys-plan.md)** — a separate file kind for **ordered, untimed** workflow documentation. Distinct from timelines (no `start_ms` / `duration_ms`) and from tags (sequence matters). v0.6.0 target.

## Capabilities at a glance

| Layer | What works as of v0.5.0 |
|---|---|
| Data | 2 schema-validated file kinds (diagrams, node-types) + optional `.loom/exports.json` for named bundles. |
| CLI | `init [--mcp]`, `install-mcp`, `view`, `validate`, `mcp`, `export-html`. |
| MCP server | 10 tools for diagrams — list/read/add-node/update/mark-stale/delete-node/add-edge/delete-edge/validate/read-node-types. All mutations schema-validated before write. |
...
| Agent skill | `.claude/skills/loom-spec/SKILL.md` with 6 worked examples + tagging hygiene + security warnings. |
| Drift detection | `loom-spec validate` walks all `code_refs[]` on nodes. Exits non-zero for CI / pre-commit. |
| Export | Standalone interactive HTML (~530 kB for the demo); tag-based filter with cascade rules (drop edges to dropped nodes, shrink groups, clear drill-downs to empty diagrams); named bundles via `.loom/exports.json`; `--diagram <id>` for single-diagram exports; explicit `--include-tag` / `--exclude-tag`. |

## Architecture

### Data model

The single source of truth is JSON files in `.loom/`. Two file kinds (plus optional config):
...
- **`.loom/exports.json`** (optional) — named export bundles. `{ exports: { "<name>": { "include-tags"?, "exclude-tags"?, diagram?, out? } } }`.

Schemas live at `packages/loom-spec/schema/{diagram,node-types}.schema.json`. TypeScript types are autogenerated via `json-schema-to-typescript`, so the runtime validator (ajv 2020) and compile-time types stay aligned.
...
└── packages/loom-spec/                         # The npm package
    ├── schema/{diagram,node-types}.schema.json
    ├── templates/                              # Scaffolded by `loom-spec init`
    │   ├── .loom/{README,node-types.json,diagrams/overview.flow.json}
    │   └── .claude/skills/loom-spec/SKILL.md
    ├── scripts/{generate-types,validate-examples,smoke-export-html}.ts
    └── src/
        ├── cli/{index,init,view,validate,mcp,installMcp,exportHtml,mcpConfig}.ts
        ├── server/{app,findLoomRoot,fileOps,watch,drift,exportFilter,exportConfig}.ts
        ├── mcp/server.ts                       # MCP stdio server
        ├── view/                               # Vite-built React + xyflow SPA
        │   ├── App.tsx                         # thin router (DiagramView)
        ...
        │   ├── state.ts
        │   ├── useViewState.ts                 # URL-hash routing: #diagram:x
        │   ├── useDiagramsList.ts
        │   ├── validate-client.ts              # inline schema validation
        │   ├── loadDiagram.ts                  # client API wrappers
...
`PulseEdge.tsx` is kept (originally built for timeline edge animations) because it'll be repurposed for Journey step transitions — see [`journeys-plan.md`](./journeys-plan.md).

### One view mode, two render contexts

- `App.tsx` is a thin router. It reads `useViewState()` (which parses `location.hash` like `#diagram:overview`) and renders `DiagramView`. The `kind` field on `ViewState` is `"diagram"` only today; it stays as a discriminator so Journeys can land cleanly later with `kind: "journey"`.
...
`loom-spec mcp` boots a stdio MCP server (using `@modelcontextprotocol/sdk`) that exposes 10 tools, all for editing diagrams. Each mutation re-reads, applies the patch, validates against the schema, and writes — so invalid edits never corrupt the file.
...
- `loom-spec validate` (or `loom_validate` MCP tool) walks every `code_refs` and reports missing files / symbols / out-of-range line refs. Skips `planned` and `deprecated` nodes since their code may legitimately not exist.

## What's verified

End-to-end checks that currently pass (via `pnpm --filter loom-spec typecheck` plus the smoke test in `packages/loom-spec/scripts/`):

- Schemas validate the example fixtures.
- TypeScript types autogenerate and the entire codebase typechecks cleanly.
- **Export-html**: smoke test (`scripts/smoke-export-html.ts`) — full export / `--diagram` / `--include-tag` cascade / `--exclude-tag` / named bundle from `.loom/exports.json` / unknown-bundle error.
