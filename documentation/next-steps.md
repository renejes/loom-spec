...
_Nothing in flight._ v0.9.0 added JUCE / audio support — RT-safety
lint, C++ signatures, signal-typed edges, wiring validation
([Phase 10](./done/phase-10-juce-rt-safety.md)). v0.8.1 was a
publish-hygiene fix ([Phase 9](./done/phase-9-publish-fix.md)).
Pick the next item from the backlog when a real pain point shows up.
...
| #25 | Add more languages to signature-drift | varies per language | v0.9.0 ships Python, TS (incl. JSX, JS), Rust, Svelte, C/C++. Other languages (Go, Java, Kotlin, C#, Ruby) silently skip the check. Each new language is a small regex+state-machine module — copy `python.ts` or `cpp.ts` and adapt. PRs welcome. |
| #28 | RT-safety: cross-call analysis | ~1 d | The RT scanner (Phase 10) checks only the function you mark `realtime`, not helpers it calls. Following one level of calls into same-file helpers would catch allocations hidden behind `doWork()`. Defer until a real bug hides behind a helper. |
| #29 | Latency / PDC + channel-count validation | ~1 d | Audio-specific: declare per-node `latency_samples`, validate the sum matches the plugin's reported latency; flag mono→stereo / stereo→mono port mismatches. Niche until the pain shows up. |
| #30 | RT-safety for more languages | varies | The body-scan + pattern approach generalizes (Rust `unsafe`/alloc in real-time threads, etc.). C/C++ only today. |
