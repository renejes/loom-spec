...
| [Phase 9](./phase-9-publish-fix.md) | **Bug-fix release**. v0.8.0 shipped with a stale `dist/` — the Phase 7+8 modules weren't actually in the published tarball, so signature drift and edge vocab were dead code on user installs. Added `prepublishOnly` hook + `check-dist` script to guarantee future publishes contain expected files. | v0.8.1 |
| [Phase 10](./phase-10-juce-rt-safety.md) | JUCE / audio: real-time-safety lint (scans `realtime`-marked C++ function bodies for heap alloc / blocking locks / juce::String / logging in the audio thread, function-body-scoped so GUI-method locks don't false-positive), C++ signature extractor (signature drift now works on .cpp/.h), signal-typed edge coloring, and edge wiring validation (port existence + signal compatibility). channelstrip plugin bootstrapped + validated clean. | v0.9.0 |
